...
def cholesky_factorization(matrix):
    """
    Perform Cholesky factorization on a symmetric positive-definite matrix.

    Parameters:
        matrix (numpy.ndarray): The input matrix.

    Returns:
        numpy.ndarray: Lower triangular matrix L such that matrix = L @ L.T
    """
    n = matrix.shape[0]
...
matrix_for_cholesky = np.array([
    [2, 1],
    [1, 4]
], dtype=float)

L = cholesky_factorization(matrix_for_cholesky)
